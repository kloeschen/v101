---
name: Boppin'B im ASB-Bahnhof Barsinghausen
aliases: [Boppin B Barsinghausen 2026, Boppin'B 40 Jahre Barsinghausen]
kurzbeschreibung: Konzert der Aschaffenburger Rock'n'Roll- und Rockabilly-Band Boppin'B am Samstag, 3. Oktober 2026, im ASB-Bahnhof Barsinghausen bei Hannover.
status: veroeffentlicht
erstelltAm: 2026-09-23
geprueftAm: 2026-09-23
geaendertAm: 2026-09-23
autor: markus
typ: konzert
beginn: 2026-10-03T20:15:00+02:00
ort: asb-bahnhof-barsinghausen
region: niedersachsen
lineupBands: [boppin-b]
preise:
  - bezeichnung: Vorverkauf im ASB-Bahnhof
    betrag: 20
    waehrung: EUR
  - bezeichnung: Abendkasse
    betrag: 25
    waehrung: EUR
eintritt: beziffert
ticketUrl: https://asb-bahnhof.reservix.de/
genres: [rockabilly, rocknroll]
durchfuehrung: geplant
links:
  website: https://www.asb-bahnhof-barsinghausen.de/2026/05/29/03-10-2026-boppin-b/
redaktionsnotiz: >-
  AKTUALITAETSBELEG: Zwei voneinander unabhaengige Quellen nennen denselben
  Termin. Die Ankuendigung des Hauses nennt "Samstag, 03. Oktober 2026, um
  20:15 Uhr (Einlass ab 19:15 Uhr)", und der 3. Oktober 2026 ist ein
  Samstag. Die Terminliste der Band bei Reservix fuehrt "Sa. 03.10.2026,
  20:15 Uhr, Barsinghausen, ASB-Bahnhof" -- gleiche Zeit, gleicher Ort.
  Die Ankuendigung liegt im Uploadpfad /2026/05/, das Datum steht im
  Adresspfad; eine aeltere Ausgabe mit demselben Namen gibt es dort nicht.
  PREISE: 20 Euro im Vorverkauf direkt im ASB-Bahnhof, 25 Euro an der
  Abendkasse, beides aus der Ankuendigung des Hauses. Der Online-Verkauf
  laeuft ueber Reservix "zzgl. Gebuehren"; Reservix nennt "ab 20,00 €".
  Den Gebuehrenbetrag nennt keine Quelle, deshalb steht fuer den
  Online-Kauf kein eigener Preis im Eintrag, nur der Hinweis im Text.
  `ticketUrl` ist die Adresse, die das Haus selbst fuer den Online-Verkauf
  nennt (asb-bahnhof.reservix.de).
  `ende` ungesetzt: Keine Quelle nennt eine Endzeit. Einlass 19:15 steht
  im Text, nicht in `beginn` -- `beginn` ist der Konzertbeginn.
  BAND: Stand zuerst in `lineupWeitere`. Am 2026-09-23 hat Markus eine
  Bandseite beschlossen (`bands/boppin-b`); nach deren Freigabe steht die
  Band hier in `lineupBands`. Die Reihenfolge ist Absicht: Vorher haette
  das JSON-LD dieses freigegebenen Termins auf eine Seite gezeigt, die die
  Produktion nicht baut (Posten in OFFENE-PUNKTE.md).
  Die Schreibweise schwankt in den Quellen (BOPPIN' B, Boppin'B,
  Boppin B, boppin b.); gewaehlt ist die Form aus dem Fliesstext der
  Ankuendigung.
  genres: Die Ankuendigung nennt "Rock'n'Roll" und "Rockabilly"; beide
  haben Lexikoneintraege.
  Nicht gefuellt: veranstalter (die Seite nennt keinen eigenen
  Veranstalter neben dem Haus), kapazitaet, barrierefrei, camping.
quellen:
  - url: https://www.asb-bahnhof-barsinghausen.de/2026/05/29/03-10-2026-boppin-b/
    titel: 03.10.2026 – Boppin' B (ASB-Bahnhof Barsinghausen)
    abgerufenAm: 2026-09-23
    felder: [beginn, ort, preise, eintritt, ticketUrl, lineupBands, genres, name, kurzbeschreibung, durchfuehrung, body:termin, body:musik, body:einordnung]
    art: offiziell
  - url: https://www.reservix.de/tickets-boppinb/t3454
    titel: Boppin' B – Tickets und Termine (Reservix)
    abgerufenAm: 2026-09-23
    felder: [beginn, ort, preise, durchfuehrung, body:termin, body:einordnung]
    art: aggregator
---

Boppin'B im ASB-Bahnhof Barsinghausen ist ein Konzert der Aschaffenburger Band [Boppin'B](/bands/boppin-b/) am Samstag, dem 3. Oktober 2026, um 20:15 Uhr. Angekündigt ist ein Abend mit [Rock'n'Roll](/lexikon/rocknroll/) und [Rockabilly](/lexikon/rockabilly/) aus dem Jubiläumsprogramm zu 40 Jahren Bandgeschichte.

## Der Termin von Boppin'B in Barsinghausen

Einlass ist nach Angabe des Hauses ab 19:15 Uhr, Beginn um 20:15 Uhr. Karten kosten im Vorverkauf direkt im ASB-Bahnhof 20 Euro, an der Abendkasse 25 Euro. Online und in den Vorverkaufsstellen von Reservix gibt es sie ebenfalls, dort kommen Gebühren hinzu, deren Höhe die Ankündigung nicht nennt. Eine Endzeit gibt keine der Quellen an.

Die Terminliste der Band beim Ticketanbieter führt denselben Abend mit derselben Uhrzeit. Damit stützen zwei voneinander unabhängige Stellen das Datum — die Ankündigung des Hauses und der Kalender der Band.

## Die Musik von Boppin'B

Das Haus zählt die Band zu den erfolgreichsten Vertretern ihres Genres in Deutschland; sie steht seit 40 Jahren auf der Bühne und hat nach dieser Angabe mehr als 6.000 Konzerte gespielt. Die Ankündigung verspricht einen Abend zum Tanzen; sie verbindet nach ihrer Beschreibung die Wurzeln des Rockabilly mit eigenen Songs.

## Einordnung des Konzerts in Barsinghausen

Der [ASB-Bahnhof](/locations/asb-bahnhof-barsinghausen/) ist ein Konzertort mit breitem Programm, kein Szenehaus. Für [Niedersachsen](/regionen/niedersachsen/) ist der Abend ein Einzelkonzert im Herbst, fernab der großen Festivalwochenenden. Die Terminliste der Band nennt in derselben Saison auch Hildesheim am 25. September sowie Dortmund und Aschaffenburg.
